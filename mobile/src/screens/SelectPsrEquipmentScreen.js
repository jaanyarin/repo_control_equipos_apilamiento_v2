...
export default function SelectPsrEquipmentScreen() {
    ...
    try {
      ...
      setError(
        e.response?.data?.error ||
          e.message ||
          "No se pudieron cargar las PSR pendientes",
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  if (loading && items.length === 0)
    return <LoadingScreen message="Buscando PSR pendientes" />;

  return (
    <View style={styles.container}>
      {error ? (
        <ErrorState
          title="No se pudieron cargar las PSR"
          message={error}
          onRetry={load}
        />
      ) : (
        <FlatList
          ...
          ListHeaderComponent={
            <Text style={styles.title}>PSR con OSR sin equipo finalizado</Text>
          }
          ListEmptyComponent={
            <EmptyState
              icon="clipboard-check-outline"
              title="Sin PSR pendientes"
              subtitle="Todas las OSR disponibles ya tienen un equipo asignado."
            />
          }
          renderItem={({ item }) => {
                ...
                <AppCard style={styles.card}>
                  <View style={styles.header}>
                    ...
                    <StatusChip
                      ...
                      label={
                        item.borradorEquipoId ? "En proceso" : "Disponible"
                      }
                    />
                    <RadioButton
                      value={value}
                      status={selectedId === value ? "checked" : "unchecked"}
                    />
                  </View>
                  <Text style={styles.meta}>
                    {item.motivo || "Sin descripción"} · {item.meses ?? "-"}{" "}
                    meses
                  </Text>
                </AppCard>
              </TouchableRipple>
      ...
      <View style={[styles.footer, { bottom: insets.bottom }]}>
        <AppButton onPress={continueFlow} disabled={!selected} fullWidth>
          {selected?.borradorEquipoId
            ? "Continuar ingreso"
            : "Seleccionar e ingresar equipo"}
        </AppButton>
      </View>
    </View>
  );

...
export default function Marcas() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await api.get('/marcas').then((r) => r.data || r)
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/marcas/${editing.id}`, formData)
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/marcas/${itemToDelete.id}`)
  ...
  const handleToggle = async (item) => {
    ...
    try {
      await api.put(`/marcas/${item.id}`, { nombre: item.nombre, estadoActivo: next })
      loadData()
    } catch (err) {
      alert(err.response?.data?.message || err.message)
    }
  ...
  return (
    <Box>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography>¿Estás seguro de eliminar la marca <strong>{itemToDelete?.nombre}</strong>?</Typography>

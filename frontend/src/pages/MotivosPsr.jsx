...
export default function MotivosPsr() {
  const { user } = useApp()
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await api.get('/motivos-psr').then((r) => r.data || r)
  ...
  const openEdit = (item) => {
    setEditing(item)
    setFormData({ nombre: item.nombre, nombreCorto: item.nombreCorto })
    setDialogOpen(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/motivos-psr/${editing.id}`, formData)
      } else {
        await api.post('/motivos-psr', formData)
      }
      setDialogOpen(false)
      loadData()
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/motivos-psr/${itemToDelete.id}`)
  ...
  const handleToggle = async (item) => {
    ...
    try {
      await api.put(`/motivos-psr/${item.id}`, { nombre: item.nombre, nombreCorto: item.nombreCorto, estadoActivo: next })
      loadData()
    } catch (err) {
      alert(err.response?.data?.message || err.message)
    }
  ...
  const columns = [
    { field: 'id', label: 'ID' },
    { field: 'nombreCorto', label: 'Nombre corto' },
  ...
  const renderCard = (item) => (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
        <Box>
          <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{item.nombreCorto}</Typography>
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2.5 }}>
        <Typography variant="h4" sx={{ fontWeight: 600, fontSize: { xs: 20, md: 24 } }}>Motivos PSR</Typography>
        {canEdit ? (
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editing ? 'Editar Motivo PSR' : 'Nuevo Motivo PSR'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            <TextField label="Nombre completo" value={formData.nombre}
              onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
              required fullWidth size="small" />
            <TextField label="Nombre corto" value={formData.nombreCorto}
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography>¿Estás seguro de eliminar el motivo <strong>{itemToDelete?.nombreCorto}</strong>?</Typography>
